...
export const crearNotificacion = async ({ titulo, mensaje, destino, id_movimiento }) => {
    if (!id_movimiento) {
      console.log("Error: id_movimiento es necesario para la notificación");
      return;
    }
  
    const sql = `
      INSERT INTO notificaciones (titulo, mensaje, destino, fk_movimiento)
      VALUES ($1, $2, $3, $4)
    `;
    const result = await pool.query(sql, [titulo, mensaje, destino, id_movimiento]);
...
export const obtenerNotificaciones = async (req, res) => {
    try {
      ...
      const sql = `SELECT * FROM notificaciones WHERE destino = $1 AND leido = false ORDER BY created_at DESC`;
      ...
      console.log("Error al obtener las notificaciones en el sistema: " + error.message);
...
export const marcarNotificacionLeida = async (req, res) => {
    try {
      ...
      if (result.rowCount > 0) {
        ...
        return res.status(400).json({ message: "No se encontró la notificación" });
      }
    } catch (error) {
      console.log("Error al marcar la notificación como leída: " + error.message);
      return res.status(500).json({ message: "Error al marcar la notificación como leída" });
    }
  };
  ...
  export const responderNotificacion = async (req, res) => {
    ...
    try {
      ...
      if (respuesta === "aceptar") {
        await pool.query(`UPDATE movimientos SET aceptado = TRUE, en_proceso = FALSE, cancelado = FALSE WHERE id_movimiento = $1`, [fk_movimiento]);
      } else if (respuesta === "cancelar") {
        await pool.query(`UPDATE movimientos SET aceptado = FALSE, en_proceso = FALSE, cancelado = TRUE WHERE id_movimiento = $1`, [fk_movimiento]);
      }
  
      await pool.query(`UPDATE notificaciones SET leido = TRUE WHERE id_notificacion = $1`, [id_notificacion]);
  
      return res.status(200).json({ message: "Movimiento actualizado correctamente desde notificación" });
    } catch (error) {
